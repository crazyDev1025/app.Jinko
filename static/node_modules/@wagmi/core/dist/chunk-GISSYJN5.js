import { InjectedConnector } from "./chunk-BVC4KGLQ.js";
import {
  __privateAdd,
  __privateGet,
  __privateMethod,
...
function configureChains(
  defaultChains,
  providers2,
  { minQuorum = 1, pollingInterval = 4e3, targetQuorum = 1, stallTimeout } = {}
) {
  ...
  return {
    ...
    provider: ({ chainId }) => {
      ...
      if (!chainProviders || !chainProviders[0])
        throw new Error(
          `No providers configured for chain "${activeChain.id}"`
        );
      let provider;
...
var Client = class {
  ...
  async autoConnect() {
    ...
    const sorted = __privateGet(this, _lastUsedConnector)
      ? [...this.connectors].sort((x) =>
          x.id === __privateGet(this, _lastUsedConnector) ? -1 : 1
        )
      : this.connectors;
    let connected = false;
...
import {
  ...
  parseBytes32String as parseBytes32String2,
...
function getProvider({ chainId } = {}) {
  const client2 = getClient();
  if (chainId) return client2.getProvider({ chainId }) || client2.provider;
...
function getWebSocketProvider({ chainId } = {}) {
  ...
  if (chainId)
    return (
      client2.getWebSocketProvider({ chainId }) || client2.webSocketProvider
    );
...
async function readContracts({ allowFailure = true, contracts, overrides }) {
  try {
    ...
    const promises = () =>
      Object.entries(contractsByChainId).map(([chainId, contracts2]) =>
        multicall({
          allowFailure,
    ...
    const resultIndexes = Object.values(contractsByChainId)
      .map((contracts2) => contracts2.map(({ index }) => index))
      .flat();
...
function watchContractEvent(
  { address, abi, chainId, eventName, once },
  callback
) {
...
async function fetchFeeData({ chainId, formatUnits: units2 = "wei" } = {}) {
  ...
  const formatted = {
    ...
    maxFeePerGas: feeData.maxFeePerGas
      ? formatUnits2(feeData.maxFeePerGas, units2)
      : null,
    maxPriorityFeePerGas: feeData.maxPriorityFeePerGas
      ? formatUnits2(feeData.maxPriorityFeePerGas, units2)
      : null,
  };
