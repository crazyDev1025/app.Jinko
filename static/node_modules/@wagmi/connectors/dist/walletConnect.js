...
var _provider,
  ...
  _removeListeners,
  removeListeners_fn,
  _setRequestedChainsIds,
  setRequestedChainsIds_fn,
  _getRequestedChainsIds,
  getRequestedChainsIds_fn,
...
var WalletConnectConnector = class extends Connector {
  ...
  async connect({ chainId, pairingTopic } = {}) {
    try {
      ...
      const isChainsStale = __privateMethod(
        this,
        _isChainsStale,
        isChainsStale_fn
      ).call(this);
  ...
  async getProvider({ chainId } = {}) {
    if (!__privateGet(this, _provider))
      await __privateMethod(this, _createProvider, createProvider_fn).call(
        this
      );
  ...
  async switchChain(chainId) {
    ...
    try {
      const provider = await this.getProvider();
      const namespaceChains = __privateMethod(
        this,
        _getNamespaceChainsIds,
        getNamespaceChainsIds_fn
      ).call(this);
      const namespaceMethods = __privateMethod(
        this,
        _getNamespaceMethods,
        getNamespaceMethods_fn
      ).call(this);
      ...
      if (!isChainApproved && namespaceMethods.includes(ADD_ETH_CHAIN_METHOD)) {
        ...
        const requestedChains = __privateMethod(
          this,
          _getRequestedChainsIds,
          getRequestedChainsIds_fn
        ).call(this);
...
createProvider_fn = async function () {
  if (
    !__privateGet(this, _initProviderPromise) &&
    typeof window !== "undefined"
  ) {
    __privateSet(
      this,
      _initProviderPromise,
      __privateMethod(this, _initProvider, initProvider_fn).call(this)
    );
  }
...
removeListeners_fn = function () {
  if (!__privateGet(this, _provider)) return;
  __privateGet(this, _provider).removeListener(
    "accountsChanged",
...
getNamespaceChainsIds_fn = function () {
  if (!__privateGet(this, _provider)) return [];
  const chainIds = __privateGet(this, _provider).session?.namespaces[
    NAMESPACE
  ]?.chains?.map((chain) => parseInt(chain.split(":")[1] || ""));
  return chainIds ?? [];
};
_getNamespaceMethods = new WeakSet();
getNamespaceMethods_fn = function () {
  if (!__privateGet(this, _provider)) return [];
  const methods = __privateGet(this, _provider).session?.namespaces[NAMESPACE]
    ?.methods;
  return methods ?? [];
};
