...
function useSyncExternalStoreWithTracked(
  subscribe,
  getSnapshot,
  ...
  if (isPlainObject(result)) {
    ...
    Object.defineProperties(
      trackedResult,
      Object.entries(trackedResult).reduce((res, [key, value]) => {
        return {
          ...res,
          [key]: {
            configurable: false,
...
function useAccount({ onConnect, onDisconnect } = {}) {
  ...
  React7.useEffect(() => {
    if (
      !!onConnect &&
      previousStatusRef.current === void 0 &&
      status === "connected"
    )
...
function useSigner({
  ...
  React10.useEffect(() => {
    const unwatch = watchSigner({ chainId }, (signer) => {
      if (signer) queryClient.invalidateQueries(queryKey4({ chainId }));
      else queryClient.removeQueries(queryKey4({ chainId }));
    });
    return unwatch;
  }, [queryClient, chainId]);
  return signerQuery;
...
function useContractInfiniteReads({
  ...
  structuralSharing = (oldData, newData) =>
    deepEqual2(oldData, newData) ? oldData : replaceEqualDeep(oldData, newData),
  suspense,
}) {
...
function useContractRead({
  ...
  staleTime,
  structuralSharing = (oldData, newData) =>
    deepEqual3(oldData, newData)
      ? oldData
      : replaceEqualDeep2(oldData, newData),
  suspense,
  watch,
} = {}) {
  const chainId = useChainId({ chainId: chainId_ });
...
import {
  deepEqual as deepEqual4,
  parseContractResult as parseContractResult2,
  readContracts as readContracts2,
} from "@wagmi/core";
...
function useContractReads({
  ...
  staleTime,
  structuralSharing = (oldData, newData) =>
    deepEqual4(oldData, newData)
      ? oldData
      : replaceEqualDeep3(oldData, newData),
  suspense,
  watch,
} = {}) {
  const { data: blockNumber } = useBlockNumber({
    ...
    select(data) {
      const result = data.map((data2, i) => {
        ...
        return abi && functionName
          ? parseContractResult2({ abi, functionName, data: data2 })
          : data2;
      });
      return select ? select(result) : result;
    },
...
function queryKey13({ chainId, name, scopeKey }) {
  return [{ entity: "ensResolver", chainId, name, scopeKey, persist: false }];
}
...
function useSendTransaction({
  ...
  return {
    ...
    reset,
    sendTransaction:
      mode === "prepared" && !request ? void 0 : sendTransaction2,
    sendTransactionAsync:
      mode === "prepared" && !request ? void 0 : sendTransactionAsync,
    status,
    variables,
  };
}
