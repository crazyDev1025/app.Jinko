import t, { useMemo as e } from "react";
import { useFocusRing as o } from "@react-aria/focus";
import { useButton as r } from "@react-aria/button";
import { useHover as s } from "@react-aria/interactions";
import { mergeProps as i } from "@react-aria/utils";
import { warn as n } from "../utils/console";
import a from "../utils/drip";
import l from "../utils/clsx";
import u from "../use-drip";
import d from "../utils/with-defaults";
import { useDOMRef as c } from "../utils/dom";
import { __DEV__ as m } from "../utils/assertion";
import { forwardRef as p } from "../utils/system";
import { filterPropsWithGroup as f, getCssColors as h } from "./utils";
import { useButtonGroupContext as b } from "./button-group-context";
import g from "./button-icon";
import x from "./button.styles";
import { jsx as P } from "react/jsx-runtime";
import { Fragment as w } from "react/jsx-runtime";
import { jsxs as y } from "react/jsx-runtime";
const C = p((d, p) => {
  const {
      as: C,
      css: v,
      iconLeftCss: N,
      iconRightCss: F,
      onClick: B,
      onPress: j,
      onPressStart: S,
      onPressEnd: k,
      onPressChange: R,
      onPressUp: U,
      ...T
    } = d,
    A = b(),
    D = f(T, A),
    E = h(D),
    {
      flat: G,
      children: H,
      disabled: L,
      animated: V,
      light: W,
      ripple: I,
      bordered: $,
      auto: q,
      borderWeight: z,
      icon: J,
      iconRight: K,
      ghost: M,
      autoFocus: O,
      className: Q,
      ...X
    } = D,
    Y = (t) => {
      V && I && Z.current && nt(t);
    },
    Z = c(p),
    { buttonProps: _, isPressed: tt } = r(
      {
        ...T,
        isDisabled: L,
        elementType: C,
        onPress: (t) => {
          "keyboard" === t.pointerType || "virtual" === t.pointerType
            ? Y(t)
            : "undefined" != typeof window && window.event && Y(window.event),
            B &&
              (B(t), console.warn("onClick is deprecated, please use onPress")),
            null == j || j(t);
        },
        onPressStart: S,
        onPressEnd: k,
        onPressChange: R,
        onPressUp: U,
      },
      Z
    ),
    { hoverProps: et, isHovered: ot } = s({ isDisabled: L }),
    { isFocused: rt, isFocusVisible: st, focusProps: it } = o({ autoFocus: O }),
    { onClick: nt, ...at } = u(!1, Z);
  m &&
    "gradient" === D.color &&
    (G || W) &&
    n(
      "Using the gradient color on flat and light buttons will have no effect."
    );
  const lt = J || K,
    ut = 0 === t.Children.count(H),
    dt = Boolean(K),
    ct = e(
      () => (tt ? "pressed" : ot ? "hovered" : L ? "disabled" : "ready"),
      [L, ot, tt]
    ),
    mt = e(() => (dt ? F : N), [dt, F, N]);
  return y(x, {
    ref: Z,
    animated: V,
    as: C,
    auto: q,
    borderWeight: z,
    bordered: $ || M,
    className: l("nextui-button", `nextui-button--${ct}`, Q),
    css: { ...v, ...E },
    "data-state": ct,
    flat: G,
    ghost: M,
    isChildLess: ut,
    isFocusVisible: st && !L,
    isHovered: ot || (M && rt),
    isPressed: tt,
    light: W,
    ...i(_, it, et, X),
    children: [
      0 === t.Children.count(H)
        ? P(g, {
            isSingle: !0,
            css: mt,
            isAuto: q,
            isGradientButtonBorder: "gradient" === d.color && ($ || M),
            isRight: dt,
            children: lt,
          })
        : lt
        ? y(w, {
            children: [
              P(g, {
                css: mt,
                isAuto: q,
                isGradientButtonBorder: "gradient" === d.color && ($ || M),
                isRight: dt,
                isSingle: !1,
                children: lt,
              }),
              P("div", {
                className: l("nextui-button-text", {
                  "nextui-button-text-right": dt,
                  "nextui-button-text-left": !dt,
                }),
                children: H,
              }),
            ],
          })
        : P("span", { className: "nextui-button-text", children: H }),
      P(a, { className: "nextui-button-drip", color: "white", ...at }),
    ],
  });
});
m && (C.displayName = "NextUI.Button"), (C.toString = () => ".nextui-button");
export default d(C, {
  ghost: !1,
  bordered: !1,
  ripple: !0,
  animated: !0,
  disabled: !1,
  autoFocus: !1,
  auto: !1,
  className: "",
  type: "button",
});
