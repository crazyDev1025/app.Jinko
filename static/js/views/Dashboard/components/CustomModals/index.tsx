...
import { useCheckAuth, useEditOrBindEmail } from 'hooks/useUserAuth'
...
export const ChangeEmailModal: React.FC<CustomModalProps> = ({ show, onClose, onConfirm }) => {
    const intl = useIntl()
    ...
    const [authDetails, setAuthDetails] = React.useState<Auth>({
        address: "",
        email: "",
    ...
    const formik = useFormik({
        initialValues: { email: authDetails?.email || "" },
        ...
        onSubmit: async (data, { resetForm }) => {
            try {
                ...
                if (response.data.token) {
                    ...
                    notify(`${intl.formatMessage({ id: "the-email-has-been-successfully-changed", defaultMessage: "The email has been successfully changed" })}`, 'success')
                    resetForm()
                    onClose()
                    onConfirm()
    ...
    useEffect(() => {
        const getAuth = async () => {
            const checkAuthResponse = await checkAuth(content.token)
            if (checkAuthResponse.error) {
                setAuthDetails(null)
            } else {
                setAuthDetails(checkAuthResponse.data)
            }
        }
        if (content?.token) {
            getAuth()
        }
    }, [checkAuth, content?.token, removeCookie])
                        ...
                        <Flex direction="column" gap="10px" align="center" style={{ width: "100%" }}>
                            <form onSubmit={formik.handleSubmit} autoComplete="off" style={{ width: "100%", display: "flex", flexDirection: "column", gap: "10px" }}>
                                ...
                                <GeneralText size="clamp(0.725rem,2vw,0.725rem)" color="#ffffff" weight="normal" css={{ fontFamily: "var(--fontFamily1)", lineHeight: 1 }} >
                                    {`${intl.formatMessage({ id: "for-receive-the-generated-results", defaultMessage: "For receive the generated results or future notifications from us" })}`}
                                </GeneralText>
