...
type PropsWithDefaults = typeof CustomAvatarEditor3.defaultProps &
  Omit<Props, keyof typeof CustomAvatarEditor3.defaultProps>

class CustomAvatarEditor3 extends React.Component<PropsWithDefaults, State> {
  ...
  componentDidUpdate(prevProps: PropsWithDefaults, prevState: State) {
    ...
    if (this.props.frame && this.props.frame !== prevProps.frame) {
      this.loadFrame(this.props.frame)
    } else if (!this.props.frame && prevState.frame !== defaultEmptyImage) {
      this.clearImage()
    }

    const context = this.getContext()
    context.clearRect(0, 0, this.getCanvas().width, this.getCanvas().height)
    if (this.state.frame.resource) {
      this.paintFrame(context, this.state.frame, this.state.image, this.props.border)
    }
    this.paint(context)
  ...
  getImage() {
    ...
    if (frame.resource) {
      context.drawImage(frame.resource, 0, 0, canvas.width, canvas.height)
    }
    return canvas
  }
  ...
  getImageScaledToCanvas() {
    ...
    if (this.state.frame.resource) {
      this.paintFrame(canvas.getContext('2d')!, this.state.frame, this.state.image, 0, 1)
    }
  ...
  async loadFrame(file: File | string) {
    if (isFileAPISupported && file instanceof File) {
      try {
        const image = await loadImageFile(file)
        this.handleFrameReady(image)
      } catch (error) {
        this.props.onLoadFailure?.()
      }
    } else if (typeof file === 'string') {
  ...
  handleFrameReady = (image: HTMLImageElement) => {
    ...
    this.setState({ drag: false, frame: imageState })
  }
  ...
  paintFrame(context: CanvasRenderingContext2D, frame: ImageState, image: ImageState, border: number, scaleFactor = this.pixelRatio) {
    if (!frame.resource) return

    const dimensions = this.getDimensions()
    const height = dimensions.canvas.height
    const width = dimensions.canvas.width
    const position = this.calculatePosition(frame, border)
    ...
    context.scale(scaleFactor, scaleFactor)
    // context.drawImage(
    //   frame.resource, borderSizeX, borderSizeY,
    //   width - borderSizeX * 2,
    //   height - borderSizeY * 2,
    // )
    context.drawImage(
      frame.resource, 0 - 1, 0,
      width + 1,
      height,
    )
...
export default CustomAvatarEditor3